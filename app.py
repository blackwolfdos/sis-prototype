...
# --- Decorators untuk Autentikasi & Otorisasi ---
...
# --- Routes Autentikasi (Tidak Berubah) ---
@app.route('/')
def index():
    if 'user_id' not in session:
        return redirect(url_for('login'))
...
def forgot_password():
    # ... (Fungsi ini bisa dibiarkan sama seperti kode lama)
...
def admin_dashboard():
    db = get_db()
    # Query ini diperbaiki untuk mengambil nama program dari tabel Pengajar
    classes = db.execute('''
        SELECT 
            k.id, 
            k.kode_kelas, 
...
# --- (FITUR BARU) Routes Input Data untuk Admin ---
...
def input_pengajar():
    if request.method == 'POST':
        ...
        # Buat username & password default dari kode pengajar
...
def upload_csv(tipe):
    ...
    if file and file.filename.endswith('.csv'):
        ...
        for row in csv_reader:
            try:
                if tipe == 'sekolah':
                    # Asumsi urutan: kode_sekolah, nama_sekolah, jenis_sekolah
                    db.execute(
                        'INSERT INTO Sekolah (kode_sekolah, nama_sekolah, jenis_sekolah) VALUES (?, ?, ?)',
                        (row[0], row[1], row[2])
                    )
                elif tipe == 'pengajar':
                    # Asumsi urutan: kode_pengajar, nama_pengajar, program
                    ...
                    # Asumsi urutan: nama_siswa, kelas_tingkat, kode_sekolah, program
                    ...
                    # Asumsi urutan: program, pertemuan_ke, detail_materi
                    db.execute(
                        'INSERT INTO Materi (program, pertemuan_ke, detail_materi) VALUES (?, ?, ?)',
                        (row[0], row[1], row[2])
                    )
                ...
                db.rollback()
                fail_count += 1
                print(f"Gagal memproses baris: {row}. Error: {e}") # Untuk debug di terminal
...
# --- Routes Pengajar (Teacher) ---

# Di dalam app.py, ganti seluruh fungsi teacher_dashboard
...
def teacher_dashboard():
    ...
    # Query ini sekarang mengambil nama program dari tabel Program
    # berdasarkan program_id yang ada di setiap kelas.
    classes = db.execute('''
        SELECT 
            k.id, 
            k.kode_kelas, 
...
# Di dalam app.py, ganti fungsi absensi_kelas yang lama

# Di dalam app.py, ganti seluruh fungsi absensi_kelas
...
def absensi_kelas(kelas_id):
    ...
    # --- PERBAIKAN DI SINI ---
    # Ini adalah return statement yang hilang untuk GET request
...
def teacher_meeting_recap():
    ...
    # Query diubah untuk mengambil data dari tabel AbsensiSesi yang baru
    meetings = db.execute('''
        SELECT 
...
# --- Routes Siswa (Student) ---
# Di dalam app.py, ganti seluruh fungsi student_portfolio
...
# Di dalam app.py, ganti seluruh fungsi admin_jadwal_pengajar
    ...
    if request.method == 'POST':
        try:
            ...
            for kelas_id in kelas_ids:
                ...
                tanggal_mulai = request.form.get(f'tanggal_mulai_{kelas_id}') # Ambil tanggal mulai
                
                db.execute(
                    'UPDATE Kelas SET hari = ?, waktu_mulai = ?, waktu_akhir = ?, tanggal_mulai = ? WHERE id = ?',
                    (hari, waktu_mulai, waktu_akhir, tanggal_mulai, kelas_id) # Simpan tanggal mulai
                )
            db.commit()
...
# Di dalam app.py, tambahkan route baru ini

# Di dalam app.py, ganti seluruh fungsi print_student_portfolio
...
# Di dalam app.py (di bagian Routes Admin)

@app.route('/admin/kelola-sesi')
@role_required('admin')
def admin_kelola_sesi():
    db = get_db()
    # Ambil semua kelas beserta nama program dan pengajar utamanya
    classes = db.execute('''
        SELECT 
...
# Di dalam app.py, ganti seluruh fungsi admin_form_sesi_manual
...
def admin_form_sesi_manual(kelas_id):
    ...
    # --- PERBAIKAN DI SINI ---
    # Kirim variabel 'now' ke template
...
def admin_teacher_meetings():
    ...
    # Query untuk mengambil statistik ringkasan untuk setiap pengajar
    ...
    # Jika admin memilih seorang pengajar, ambil detail pertemuannya
...
# Di dalam app.py, ganti seluruh fungsi admin_create_slip_gaji
...
def admin_create_slip_gaji():
    ...
    # Asumsi tarif per sesi adalah 50000
    ...
    # --- PERBAIKAN DI SINI ---
    # Pastikan Anda me-render template 'admin_form_gaji.html'
    ...
    # Asumsi tarif per sesi adalah 50000, bisa Anda ubah
    tarif_per_sesi = 50000
    transport_value = jumlah_sesi * tarif_per_sesi
